...
public class StatGrading {
	...
	private static int test9() {
		int pointsEarned = 10;
		System.out.println("\nTest 9: Min/max/etc test\n");
		Constructor<Stat> dConstructor = null;
		Method minMethod = null, avgMethod = null,
				modeMethod = null, varMethod = null, stdMethod = null;
		Stat stat1 = null;

		double[] data1 = { -5.3, 2.5, 88.9, 0, 0.0, 28, 16.5, 88.9, 109.5,-90, 88.9, 100.34, 50.01, 50.01, -8 };

		try {
			dConstructor = Stat.class.getConstructor(new Class[] { double[].class });
		} catch (Exception e) { /* default constructor will remain null */ }
		...
		try {
			// this line is equivalent to: stat1 = new Stat();
			stat1 = dConstructor.newInstance(data1);
		} catch (Exception e) { /* stat1 will remain null */ }
		...
		try {
			...
			System.out.println("min:\t -> expected: " + reference + "\t\t found: " + (double) minMethod.invoke(stat1));
			if ((double) minMethod.invoke(stat1) != reference)
				pointsEarned -= 2;
		} catch (Exception e) {
			// if student debug mode is on, print stack trace and exit
			if (printStackTrace) {
		...
		try {
			...
			System.out.println("avg:\t -> expected: " + reference + "\t\t found: " + (double)avgMethod.invoke(stat1));
			if ((double) avgMethod.invoke(stat1) != reference)
				pointsEarned -= 2;
		...
		try {
			...
			System.out.println("mode:\t -> expected: " + reference + "\t\t found: " + (double) modeMethod.invoke(stat1));
			if ((double) modeMethod.invoke(stat1) != reference)
				pointsEarned -= 2;
		...
		try {
			...
			System.out.println("var:\t -> expected: " + reference + " found: " +  (double) varMethod.invoke(stat1));
			if ((double) varMethod.invoke(stat1) != reference)
				pointsEarned -= 2;
		...
		try {
			...
			System.out.println("stdev:\t -> expected: " + reference + "\t found: " + (double) stdMethod.invoke(stat1));
			if ((double) stdMethod.invoke(stat1) != reference)
				pointsEarned -= 2;
	...
	public static void main(String[] args) {
		...
		System.out.println("ALL TESTS PASSED: " + (v1t1 && v1t2 && v1t3 && v1t4 && v1t5 && v1t6 && v1t7 && v1t8 && v1t9 == 10 && v1t10));
