...
public static boolean addMenuItem(ArrayList<Integer> menuIds, ArrayList<Integer> productIds){
	
	if (menuIds.size() != productIds.size())
		throw new IllegalArgumentException("menuIds and productIds must be same size");
	...
	for (int i = 0; i < menuIds.size(); i++){
		...
		try {
			...
			preparedStmt.setInt(1, menuIds.get(i));
			preparedStmt.setInt(2, productIds.get(i));

			preparedStmt.execute();

			System.out.println("added menu item " + menuIds.get(i) + ", " + productIds.get(i));
...
public static boolean addProductAllergy(ArrayList<Integer> productIds, ArrayList<String> allergies){
	
	if (productIds.size() != allergies.size())
		throw new IllegalArgumentException("productIds and allergies must be the same size");
	...
	for (int i = 0; i < productIds.size(); i++){
		try {
			...
			preparedStmt.setInt(1, productIds.get(i));
			preparedStmt.setString(2, allergies.get(i));

			preparedStmt.execute();

			System.out.println("added product allergy " + productIds.get(i) + ", " + allergies.get(i));
	...
	public static ArrayList<Product> searchForProducts(String searchTerm){
		...
		//if the serach term is null, return the list of all the products
		if (searchTerm == null)
			return list;
	 ...
	 * 	if the searchTerm is null, return all products
	 */
	public static ArrayList<Product> searchForProducts(String searchTerm, int[] bakeryIds){
		if (searchTerm == null)
			return searchForProducts(null);
		
		
		ArrayList<Product> list = new ArrayList<Product>();
		
		String query = "SELECT * FROM Products p WHERE p.id IN "
				+ "(SELECT m.product_id FROM Menus m WHERE m.bakery_id = ";
		
		for (int x : bakeryIds)
			query += x + " OR m.bakery_id = ";
		
		query = query.trim().substring(0, query.length() - 19) + ");";
