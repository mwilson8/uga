...
//        index     = new LinHashMap <> (KeyType.class, Comparable [].class);	
    ...
    public Table project (String attributes)
    {
        ...
        //TODO what if a given attribute is not in the table?
        //currently returns a null Table object 
        //notice that ln 143 changes the key if all attributes aren't found? 
        for (String s: attrs){
        	if (! Arrays.asList(attribute).contains(s)){
        		return null;
        	}
        }
        ...
        for (int i = 0; i < tuples.size(); i++){
        	//extract method pulls out tuples that have the attributes contained within attrs
    ...
    public Table select (KeyType keyVal)
    {
        ...
        for (KeyType kt : index.keySet()){
        	if (kt.equals(keyVal))
        		rows.add(index.get(kt));
    ...
    public Table union (Table table2)
    {
        ...
        for (Comparable[] comp : this.tuples){
        	if (! rows.contains(comp))
        		rows.add(comp);
        }
        
        for (Comparable[] comp : table2.tuples){
        	if (! rows.contains(comp))
        		rows.add(comp);
        }
    ...
    public Table minus (Table table2)
    {
        ...
        for (Comparable[] comp : this.tuples){
        	if (! table2.tuples.contains(comp) && ! rows.contains(comp))
        		rows.add(comp);
        }
    ...
    { 
    	if (t.length != attribute.length)
        	...
        	for (int i = 0; i < t.length; i ++){
        		if (! t[i].getClass().equals( domain[i] ))
        			return false;
