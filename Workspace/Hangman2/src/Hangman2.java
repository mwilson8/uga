...
public class Hangman2 {
	...
	// Brad challenged to do it in as few lines as possible
	// crammed together, I can get it into 80 lines
	// spaced out it's closer to 120, not counting comments
	
	public static void main(String[] args) {

		boolean wantsToKeepPlaying = true; int gamesPlayed;
		
		Scanner keyboard = new Scanner(System.in);
		for (gamesPlayed = 1; (gamesPlayed <= 20 && wantsToKeepPlaying == true); gamesPlayed++) {
			// main program loop: executes as long as there are words left (gamesPlayed <= 20) &&
			// the user wants to continue (wantsToKeepPlaying == true)
			
			int spaces = 2, guesses = 10, indexGuess = 0;
			// initializing spaces & guesses for 'hard' difficulty
			...
			do {
				...
				if (difficulty.charAt(0) == 'h' || difficulty.charAt(0) == 'i' || difficulty.charAt(0) == 'e') {
					// if the difficulty starts with 'h', 'i', or 'e' go into here
					
					difficultyCheck = true;
					if (difficulty.charAt(0) != 'h') {
						// if the difficulty is not 'h' (so it's 'e' or 'i')
				...
				// difficultyCheck is only true when difficulty is either e, i, or h
			...
			// if enabled, display the secret word
			...
			while (guessesRemaining > 0 && !displayWord.equals(secretWord)) {
				...
				// they haven't guessed the secret word yet
				...
				while (inputIsALetter == false) {
					...
					if (Character.isDigit(guessChar)) {
						...
						// if input is solve, set inputIsALetter to true, so we exit loop
						...
						// input is a letter, and we exit the loop
				}
				// end (while) loop for inputIsALetter?
				
				
				if(!guessIsSolve) System.out.println("What spaces do you want to check?");
				// if they didn't guess solve, we proceed to ask for spaces
				
				String spacesString = "";
				boolean spacesCheckPassed = false;
				while (spacesCheckPassed == false && guessIsSolve == false) {
					// spaces validation loop executes until spacesCheckPassed == true &&
					// if guess isn't solve
					...
					// need space on the end for following loop, because we check 2 spaces at a time, unconditionally
					...
					for (int index = 0; index < spacesString.length() && keepChecking; index += 2) {
						...
						// until we check the entire word
						...
						if (!Character.isDigit(spacesString.charAt(index)) || (spacesString.charAt(index + 1) != ' ')
							...
							// otherwise, check the next 2 chars (increase index by 2)
					}
					// end (for) keepChecking loop
					
					if (keepChecking)
						spacesCheckPassed = true;
					// if keepChecking == true, it means we exited the check loop becuase we checked the whole word
					
				}
				// end (while) spacesCheckPassed loop
				...
				for (int indexForCountLoop = 0, count1 = 0; count1 < spaces && !guessIsSolve; indexForCountLoop += 2, count1++) {
					...
					// indexGuess is the numeric value of the digit we're checking
			...
			// end (while) game loop
			// if exiting this loop, guessesRemaining == 0 or they've guessed the word
			

			for (boolean keepPlayingCheck = false; keepPlayingCheck == false;) {
		...
		// end (for) main program loop
		// if we are here, we are either out of words, or they don't want to keep playing
		// if we are here, we are exiting regardless
