...
//import JukeBoxControls.ButtonListener;
    ...
    extends JFrame
{
    ...
    private MP3File 				   current;
    private static MP3Collection	   collection;
    private JButton stopButton, playButton, cancelButton;
  
    // construct the panel and components for this frame
    MyTunes( String title, MP3Collection collection )
    {
	// set the frame title
	super (title);
	...
	model = new DefaultTableModel( collection.getTableData(), columns ) {
		@Override
	...
	*/
	...
	JButton setFilter = new JButton( "Search" );
    ...
    private class ButtonListener implements ActionListener
    {
       ...
       { 
    	   ...
    	   if (event.getSource() == playButton || event.getSource() == stopButton){
				...
				if (event.getSource() == playButton) {
					current = collection.getFile(row);
				
					if (current != null) {
						collection.startPlay(row);
						System.out.println("Playing: " + collection.getFile(row).getTitle());
	...
	Runnable r = new Runnable () {
		public void run () {
		    new MyTunes( "Dow Industrial Companies" );
		}
	    };
    ...
    public static void main( String [] args )
    {
		...
		if (args.length > 0)
		{
			...
			if (mp3Dir.isDirectory())
				collection = new MP3Collection(args[0]);
			...
			collection = new MP3Collection("./Music");
    	}
    	catch (Exception e)
    	{
    		System.err.println("Error: could not load directory");
    	}
		...
		@Override 
		    public void run() {
		    MyTunes MyTunesGUI = new MyTunes( "MyTunes", collection ); //take in a title & mp3 collection
		    MyTunesGUI.setVisible(true);
		}
	    } );
    }
}
